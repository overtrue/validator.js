...
(function() {
  ...
  function _addError(attribute, rule, parameters) {
    ...
    var message = _doReplacements(_getMessage(attribute, rule) || '', attribute, rule, parameters);

    is_object(_errors[attribute]) || (_errors[attribute] = {});

    _errors[attribute][rule] = message;
   ...
   * @return {String}
   */
  function _doReplacements(message, attribute, rule, parameters) {
    message = message.replace(':attribute', _getAttribute(attribute))
  ...
  function _getAttribute(attribute) {
    if (typeof _customAttributes[attribute] == 'string') {
      return _customAttributes[attribute];
    }

    if ((line = translator.trans(attribute)) !== attribute) {
      return line;
    } else {
  ...
  function _anyFailingRequired(attributes) {
    for (var i in attributes) {
      ...
      if ( ! resolvers.validateRequired(key, self.date[key])) {
        return true;
      }
    }
  ...
  var _resolvers = {
    accepted: function(attribute, value) {
      ...
      return (resolvers.required(attribute, value) && in_array(value, acceptable, true));
    },
    ...
    confirmed: function(attribute, value, parameters) {
      return _resolvers.same(attribute, value, [attribute + '_confirmation']);
    },
  ...
  Validator.prototype = {
    ...
    mergeValues: function(value, alias) {
      if (is_object(value)) {
        _customValues = extend({}, _customValues, value);
      } else if (is_string(rule)) {
        _customValues[value] = alias;
      } 
    },
     ...
     * @return {Void}
     */
    register: function(rule, fn, errMsg) {
      _resolvers[rule] = fn;
     ...
     * @return {Object} 
     */
    make: function(data, rule, messages){
